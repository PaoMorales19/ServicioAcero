...
public class JmsProducer {
    ...
    private ConnectionFactory connectionFactory;  // La fábrica de conexiones para conectarse al broker JMS
    ...
    private MessageProducer producer;  // El productor que enviará mensajes a la cola

    // Constructor de la clase JmsProducer que lanza posibles excepciones relacionadas con JMS y JNDI
    public JmsProducer() throws JMSException, NamingException {

        // Configuración del contexto JNDI, que nos permite buscar y utilizar recursos remotos (como colas)
        Properties env = new Properties();
        // Se especifica la fábrica de contexto inicial de ActiveMQ
        env.put(Context.INITIAL_CONTEXT_FACTORY, "org.apache.activemq.jndi.ActiveMQInitialContextFactory");
        // URL del proveedor del broker JMS (ActiveMQ en este caso), que es un servidor en la dirección TCP localhost
        ...
        // Buscamos la fábrica de conexiones (ConnectionFactory) en el contexto JNDI para conectar con el broker
        connectionFactory = (ConnectionFactory) jndiContext.lookup("ConnectionFactory");

        // Creamos una conexión utilizando la fábrica obtenida y la iniciamos
        connection = connectionFactory.createConnection();
        connection.start();  // Se inicia la conexión para enviar/recibir mensajes

        // Creamos una sesión sin transacciones (false) y con reconocimiento automático de mensajes
        session = connection.createSession(false, Session.AUTO_ACKNOWLEDGE);

        // Buscamos la cola (queue) definida en JNDI con el nombre "amq.compras.in"
        destination = (Destination) jndiContext.lookup("amq.compras.in");

        // Creamos un productor para enviar mensajes a la cola obtenida
    ...
    public void sendMessage(String messageText) throws JMSException {
        ...
        // Establecemos el tipo de mensaje, en este caso se indica que es un mensaje de tipo JSON
        ...
        // - Modo de entrega no persistente (NON_PERSISTENT), lo que significa que el mensaje no se almacenará en el disco
        // - Prioridad alta (9) para darle mayor prioridad en la cola
        // - Tiempo de vida del mensaje de 100,000 milisegundos (100 segundos) antes de expirar
    ...
    // Método para cerrar todos los recursos relacionados con JMS cuando ya no sean necesarios
